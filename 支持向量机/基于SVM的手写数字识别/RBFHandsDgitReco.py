...
def loadImages(dirName):
    hwLabels = []                                                      #标签列表
    trainingFileList = listdir(dirName)                                #获取文件列表
    m = len(trainingFileList)                                          #获取文件数量
    trainingMat = numpy.zeros((m,1024))                                #为数据集导入做准备
    for i in range(m):                                                 #轮询文件
        fileNameStr = trainingFileList[i]                              #获得名字
        fileStr = fileNameStr.split('.')[0]                            #分割，比如9_20.txt,去除txt
        classNumStr = int(fileStr.split('_')[0])                       #获得9_20中的9
        if classNumStr == 9:                                           #由于该分类器是二分类的分类器，数据为9则记为-1，为其他则记为1
            hwLabels.append(-1)
        else:                                                          #非9数据标记为1
            hwLabels.append(1)
        trainingMat[i,:] = img2vector('%s/%s' % (dirName,fileNameStr)) #导入数据
